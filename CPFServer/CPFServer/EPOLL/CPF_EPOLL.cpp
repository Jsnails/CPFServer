                ...
                if (events[i].events&EPOLLIN)    //有连接到来
                {
                    do
                    {
                        ...
                        if (connfd > 0)
                        {
							...
							cout << "AcceptThread, write success"<< endl;
                        }
                        else
                        {
...
void * CPF_EPOLL::ReadThread(void *pUser)
{
    ...
    while (pThis->m_bRun)
    {
        ...
        do
        {
            ...
            {
                //队列中的fd必须是有效的
				cout << "read pipe size:,msg.fd=%d,msg.op=%d" <<ret<<msg.fd<<msg.op<< endl;
                if (ret == 14 && msg.fd > 0)
                {
                    if (msg.op == 1)    //收到新的连接
                    {
                        ...
                        else
                        {
							...
							OnHandleIO(OP_ACCEPT, msg.fd, buf, 0, pThis);
                        }
                    }
